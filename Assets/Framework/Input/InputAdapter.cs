...
public class InputAdapter : Singleton<InputAdapter>
{
    ...
    [CanBeNull] private static SettingsData _settingsData;
    ...
    private void Awake()
    {
        this.RunTaskInCoroutine(PlayerDataStore.GetOrCreate(), () => _settingsData = PlayerDataStore.GetOrCreate().Result.Settings);
        ...
        Drag = new PriorityAction<Vector2>(_controls.Standard.Drag, x => x * (_settingsData?.DragSpeed ?? 1));
        Scroll = new PriorityAction<float>(_controls.Standard.Scroll, x => x * (_settingsData?.ScrollSpeed ?? 1));
