...
public class CameraController : Singleton<CameraController>
{
    ...
    private static float GetTransitionTime(Vector3 start, Vector3 end, float transitionSpeed = 1) => math.sqrt(Vector3.Distance(start, end)) * 0.05f / transitionSpeed;
    private static float GetTransitionTime(Quaternion start, Quaternion end, float transitionSpeed = 1) => math.sqrt(Quaternion.Angle(start, end)) * 0.05f / transitionSpeed;
    private static float GetTransitionTime(float start, float end, float transitionSpeed = 1) => math.sqrt(math.abs(start - end)) * 0.05f / transitionSpeed;
}