...
public struct CartesianCoord
{
    ...
    {
        XYZ = new float3(
            math.cos(sc.Phi) * math.cos(sc.Theta) * sc.Altitude,
            math.sin(sc.Phi) * sc.Altitude,
            math.cos(sc.Phi) * math.sin(sc.Theta) * sc.Altitude);
    }
...
public struct SphericalCoord
{
    ...
    public SphericalCoord(float theta, float phi, float altitude)
    {
        Theta = theta;
        Phi = phi;
        Altitude = altitude;
    ...
    {
        Altitude = math.sqrt(math.pow(cc.X, 2) + math.pow(cc.Y, 2) + math.pow(cc.Z, 2));
        Theta = math.acos(cc.Z / Altitude);
