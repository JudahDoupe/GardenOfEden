...
public static class PlayerDataStore
{
    ...
    private static readonly string _path = $"{Application.persistentDataPath}/Player.data";
    ...
    public static async Task<PlayerData> GetOrCreate()
    {
        ...
        var dbData = await LoadDataAsync<PlayerDbData>(_path);
        if (dbData != null)
        {
    ...
    {
        await SaveDataAsync(_path, data.ToDbData());
    }
