...
namespace Assets.Scripts.Utils
{
    public static class AnimationUtils
    {
        ...
        {
            Singleton.Instance.StartCoroutine(AnimateVector3(seconds, transform.localPosition, localPosition, pos => transform.localPosition = pos, callback, ease));
        }
        ...
        {
            Singleton.Instance.StartCoroutine(AnimateVector3(seconds, transform.localScale, localScale, scale => transform.localScale = scale, callback, ease));
        }
        ...
        {
            Singleton.Instance.StartCoroutine(AnimateQuaternion(seconds, transform.localRotation, localRotation, rot => transform.localRotation = rot, callback, ease));
        }
        ...
        {
            Singleton.Instance.StartCoroutine(AnimateFloat(seconds, camera.fieldOfView, fov, f => camera.fieldOfView = f, callback, ease));
        }
        public static void AnimateOpacity(this Transform transform, float seconds, float alpha, EaseType ease = EaseType.Linear)
        {
            ...
            Singleton.Instance.StartCoroutine(AnimateFloat(seconds, material.color.a, alpha, a => material.color = new Color(material.color.r, material.color.g, material.color.b, a), ease: ease));
        }
        public static void AnimateUiOpacity(this Transform transform, float seconds, float alpha, EaseType ease = EaseType.Linear)
        {
            ...
            {
                Singleton.Instance.StartCoroutine(AnimateFloat(seconds, image.color.a, alpha, a => image.color = new Color(image.color.r, image.color.g, image.color.b, a), ease: ease));
            }
            foreach (var text in transform.GetComponentsInChildren<Text>())
            {
                Singleton.Instance.StartCoroutine(AnimateFloat(seconds, text.color.a, alpha, a => text.color = new Color(text.color.r, text.color.g, text.color.b, a), ease: ease));
            }
        }
